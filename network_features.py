...
class NetworkFeatures:
    ...
    def load_network(self, network_filename: str, total_edges: int):
        ...
        if network_filename.endswith("csv"):
            with open(network_filename,'r', encoding='latin1') as csv_file:
            ...
            network = from_edge_list(edges, directed=True)

            return network
        else:
            with gzip.open(network_filename,'rt', encoding='latin1') as csv_file:
                reader = csv.reader(csv_file)
                next(reader)
    ...
    def calculate_page_rank(self, graph, damping_factor=0.85, iterations=100) -> list[float]:
        ...
        rank = PageRank(damping_factor=damping_factor, n_iter=iterations)
        scores = rank.fit_predict(graph.adjacency)
    ...
    def calculate_hits(self, graph) -> tuple[list[float], list[float]]:
        ...
        rank = HITS()
        hits = rank.fit(graph.adjacency)
        hub = hits.scores_row_
        authority = hits.scores_col_
    ...
    def get_all_network_statistics(self, graph) -> DataFrame:
        ...
        data = {}
        data["docid"] = [i for i in sorted(graph.names)]
        data["pagerank"] = self.calculate_page_rank(graph)

        hits = self.calculate_hits(graph)
        data["authority_score"] = hits[1]
        data["hub_score"] = hits[0]
